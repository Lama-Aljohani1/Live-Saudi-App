...
class AdminManageHeritageController extends GetxController {
  ...
  final RxList<Map<String, dynamic>> currentReviews =
  ...
  Future<void> updateSite(String id, {required HeritageModel oldSite}) async {
    try {
      ...
      final updatedData = {
        ...
        'region': region.text.trim().isEmpty
            ? oldSite.region
            : region.text.trim(),
  ...
  void loadReviews(String siteId) async {
    ...
    if (doc.exists && doc.data() != null) {
      final data = doc.data()!;
      currentReviews.value = List<Map<String, dynamic>>.from(
        data['reviews'] ?? [],
      );
    }
  }
  ...
  Future<void> addReview(String siteId) async {
    ...
    if (currentReviews.isNotEmpty) {
      double total = currentReviews.fold(
        0,
        (sum, r) => sum + (r['stars'] ?? 0),
      );
    ...
    await FirebaseFirestore.instance
        .collection('heritage_sites')
        .doc(siteId)
        .update({'reviews': currentReviews, 'rating': avgRating});
